...
do
  ...
  echo "File ${filename}"
  "${BASH_HOME}aux_valxsd.sh" "${csxfile}" "${file}"
  if [[ "$?" -ne "0" ]]; then
...
# DONT Apply XSLT transformations to postprocess to ensure sequential indexing of edges with explicit index, like content
...
#  "${BASH_HOME}aux_xslt.sh" "${f}" "${VALIDATOR_XSLT_HOME}lrml_validator-sequential-indexing.xslt" "${fnew}"
  ...
  "${BASH_HOME}aux_valxsd.sh" "${csxfile}" "${fnew}"
  if [[ "$?" -ne "0" ]]; then
...
do
  ...
  "${BASH_HOME}aux_valxsd.sh" "${nsxfile}" "${fnew}"
  if [[ "$?" -ne "0" ]]; then
