...
for f in "${REPO_HOME}examples/approved/"*.lrml "${REPO_HOME}examples/draft/"*.lrml "${REPO_HOME}examples/tutorial/"*.lrml
do
  filename=$(basename "$f")
...
# Law: If w satisfies the compact schemas then SSw = Sw
...
# Corr: If y satisfies the compact schema and the
...
# transform files in INSTANCE_SEQ_COMPACT_HOME
# Law: If w satisfies the compact schemas then 
#            VVw = Vw
...
do
  ...
  fnewer="${INSTANCE_SEQ_COMPACT_HOME}pre-pre-${filename}"
  "${BASH_HOME}aux_xslt.sh" "${f}" "${NORMAL_XSLT_HOME}lrml_prefix_evaluation.xslt" "${fnew}"
  "${BASH_HOME}aux_xslt.sh" "${fnew}" "${NORMAL_XSLT_HOME}lrml_prefix_evaluation.xslt" "${fnewer}"
  #"${BASH_HOME}aux_xslt.sh" "${fnew}" "${INSTANCE_XSLT_HOME}lrml_instance-postprocessor-stripwhitespace.xslt" "${fnew}"
  read -r firstlineold<"${fnew}"
  read -r firstlinenew<"${fnewer}"
...
# transform files in INSTANCE_SEQ_COMPACT_HOME
# Law: If y satisfies the compact schemas and 
...
# Corr 1: If w satisfies the compact schemas, then 
#         CSw = Sw
for f in "${INSTANCE_SEQ_COMPACT_HOME}"*.lrml
do
  filename=$(basename "$f")
...
# Law: If y satisfies the compact XSD schema and 
#                     the additional restrictions enforced by sequential-indexing, 
#      then CNy = y
# That is, CNSw = Sw for xsd-compact conformant w.
for f in "${INSTANCE_SEQ_COMPACT_HOME}"*.lrml
do
  filename=$(basename "$f")
...
# Apply XSLT normalization after sequential indexing
# The result should satisfy the sequential indexing constraints.
# Law: If y satisfies the compact XSD schema and 
#                     the additional restrictions enforced by sequential-indexing, 
#      then SNy = Ny
# Corr: SNSw = NSw if y satisfies the compact XSD schemas
#
...
# Law: If y satisfies the compact schemas then BBy = By
...
# Law: If y satisfies the compact XSD schema, 
#      then BNy = NBy
for f in "${INSTANCE_COMPACT_HOME}"*.lrml
do
  filename=$(basename "$f")
...
# Need To Show: The following transformations are abstract-syntax preserving on compact-XSD conformant LegalRuleML instances
#   B, T, ST, C, N
#   while S alone is not abstract-syntax preserving. 
#   E.g. B is abstract-syntax preserving for some set of compact-XSD conformant instance documents A iff
#    FBv = Fv for all v in A

# B = transformation that enforces the leaf-branch edge constraint
# Need to show FBx = Fx for all XSD-compact conformant x
...
# Need to show FTx = Fx for all XSD-compact conformant x
...
# Need to show FSTx = Fx for all XSD-compact conformant x
...
# S = transformation that enforces sequential indexing without sorting first
# Need to show FSx != Fx for all XSD-compact conformant x where Tx != x
...
# N = normalizer
# Need to show FNx = Fx for all XSD-compact conformant x
...
# C = compactifier
# Need to show FCx = Fx for all XSD-compact conformant x
