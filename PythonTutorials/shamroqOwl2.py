##                """
##                Privilege: {<PRP>?<VB|VBD|VBZ>*<RB|RBR>?}
##                No-Right: {<PRP>?<VB|VBD|VBZ>*<RB|RBR>?}
##                Power: {<PRP>?<VB|VBD|VBZ>*<RB|RBR>?}
##                Liability: {<PRP>?<VB|VBD|VBZ>*<RB|RBR>?}
##                Immunity: {<PRP>?<VB|VBD|VBZ>*<RB|RBR>?}
##                Disability: {<PRP>?<VB|VBD|VBZ>*<RB|RBR>?}
##
"""
    right = 
                does not have a right to
                authorize termination of, must obtain an authorization, may revoke, may terminate
                provide that <actor> will/must, obtain assurance None may not authorize
             
    right3 = 'John retains the right to permint' 
 """
"""
    ...
    try:
        for regSent in tokenizedRegs:
            ...
            """
                ...
                is subject to
            """
            ## Order matter. If a pattern is found, it kicks out
            hohfeldianCFG2 = r"""
                Obligation: {<MD><RB>}
                Obligation: {<MD>} 
                Obligation: {<VBZ><VBN><TO>?}
                Obligation: {<MD><RB>}
                Obligation: {<VBZ><JJ><TO>?}
                """;

            ## Order matter. If a pattern is found, it kicks out
            hohfeldianCFG3 = r"""
                Privilege: {<MD><VB><RB><TO>}
                Privilege: {<MD>} 
                Privilege: {<VBZ><RB><VBN><TO>}
                Privilege: {<NN><VBZ><RB><VB>}
                Privilege: {<VBZ><VBN><TO>}
                Privilege: {<IN><DT><NN><IN>}
                Privilege: {<VBZ><RB><JJ><TO>}
                """;
            ...
            ## Order matter. If a pattern is found, it kicks out
            hohfeldianCFG4 = r"""
                NO-Right: {<VBZ><RB><VB><DT><NN><TO>}
                """;

            ## Order matter. If a pattern is found, it kicks out
            hohfeldianCFGFINAL = r"""
                Right: {<VBZ>?<DT>?<NN><TO>}
                Obligation: {<MD><RB>}
                Obligation: {<MD>} 
                Obligation: {<VBZ><VBN><TO>?}
